...
export default function Profile() {
  ...
  const loadProfile = async () => {
    try {
      ...
      setIsError(true)
      setStatusMessage(error.message || 'No pudimos cargar tu perfil.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSaveName = async () => {
    ...
    try {
      ...
      setProfile(response.profile)
      setStatusMessage('Nombre del negocio actualizado.')
      setIsError(false)
    } catch (error) {
      if (isAuthError(error.message)) {
        clearSessionAndRedirect(navigate)
      ...
      setIsError(true)
      setStatusMessage(error.message || 'No se pudo actualizar el nombre del negocio.')
    } finally {
  ...
  const handleSaveDescription = async () => {
    ...
    try {
      ...
      setProfile(response.profile)
      setStatusMessage('Descripción del negocio actualizada.')
      setIsError(false)
    } catch (error) {
      if (isAuthError(error.message)) {
        clearSessionAndRedirect(navigate)
      ...
      setIsError(true)
      setStatusMessage(error.message || 'No se pudo actualizar la descripción del negocio.')
    } finally {
  ...
  const handlePhotoChange = async (event) => {
    ...
    try {
      ...
      setProfile(response.profile)
      setIsError(false)
      setStatusMessage('Foto del negocio actualizada.')
      setIsPhotoMenuOpen(false)
    } catch (error) {
      if (isAuthError(error.message)) {
        clearSessionAndRedirect(navigate)
        ...
        <div className={styles.card}>
          ...
          {isError && (
            <div className={styles.errorBox}>
              <p>{statusMessage || 'Hubo un error al cargar tu perfil.'}</p>
...
function isAuthError(message = '') {
  return /token|sesión|sesion|expirado|unauthorized|401/i.test(String(message))
}
